...
include_once "bd.inc.php";
    ...
    function getTraverseesByDate($date){
        $resultat = array();
        try {
            $cnx = getPDO();
            $req = $cnx->prepare("SELECT * FROM traversee T JOIN bateau B ON B.id=T.idBateau
    ...
    function getPlacesDispoTraverseesByTraverseeANDCat($idTraversee, $cat, $date){
        $Traversees = getTraverseesByDate($date);
        var_dump($Traversees);
        $trav_place = getPlacesReservesTraversees();
        foreach($Traversees as $trav){
            $placeByTrav = getPlacesTraverseesByLiaisonAndDate($trav["codeLiaison"], $date);
            if(count($placeByTrav[$idTraversee]) != 0){
                if(count($trav_place[$idTraversee]) != 0){
